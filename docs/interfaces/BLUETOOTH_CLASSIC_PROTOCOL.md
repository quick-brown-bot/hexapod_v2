# Bluetooth Classic SPP Controller Protocol (DEPRECATED)

**This binary protocol is deprecated and no longer in use.**

The Bluetooth Classic controller now communicates with the hexapod locomotion framework using the [RPC System](./RPC_SYSTEM_DESIGN.md). Control data is sent as a text-based command.

## New RPC-Based Protocol

Instead of a binary frame, the controller sends a command string to the RPC system over the Bluetooth SPP connection.
